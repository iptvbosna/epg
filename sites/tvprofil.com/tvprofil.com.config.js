...
const USE_PROXY = process.env.NODE_ENV !== 'test' //  ⬅️ test = bez proxy
...
module.exports = {
  ...
  url: function ({ channel, date }) {
    ...
    if (!USE_PROXY) return targetUrl //  ⬅️ test traži originalni URL bez proxy-ja
  ...
  async channels() {
    ...
    for (let country in countries) {
      ...
      const url = USE_PROXY
        ? `${WORKER_URL}?url=${encodeURIComponent(targetUrl)}&callback=cb`
        : `${targetUrl}?callback=cb`
